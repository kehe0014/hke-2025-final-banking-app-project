...
if [[ -f "$MANIFEST_PATH/kustomization.yaml" ]]; then
    ...
    sed -i "s|tdksoft341/tdk-banking-app:[^\\\"]*|tdksoft341/tdk-banking-app:$IMAGE_TAG|g" "$MANIFEST_PATH/kustomization.yaml"
    
    # Alternative: mise à jour via yq si disponible
    if command -v yq &> /dev/null; then
        yq eval ".images[] |= select(.name == \"tdksoft341/tdk-banking-app\").newTag = \"$IMAGE_TAG\"" -i "$MANIFEST_PATH/kustomization.yaml"
    fi
